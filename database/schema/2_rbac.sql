...
-- User-Permission mapping (many-to-many) for direct permissions
...
CREATE
OR REPLACE FUNCTION permission_matches(pattern TEXT, permission TEXT) RETURNS BOOLEAN AS $$ DECLARE pattern_parts TEXT [];

permission_parts TEXT [];

BEGIN -- Split pattern and permission by ':'
...
IF array_length(pattern_parts, 1) != 2
OR array_length(permission_parts, 1) != 2 THEN RETURN FALSE;

END IF;

-- Check resource match (supports '*' wildcard)
IF pattern_parts [1] != '*'
AND pattern_parts [1] != permission_parts [1] THEN RETURN FALSE;

END IF;

-- Check action match (supports '*' wildcard)
IF pattern_parts [2] != '*'
AND pattern_parts [2] != permission_parts [2] THEN RETURN FALSE;
...
-- Checks if user has a specific permission directly (via user_permissions) or through roles
CREATE
OR REPLACE FUNCTION user_has_permission(
    user_id_param UUID,
    required_resource TEXT,
    required_action TEXT
) RETURNS BOOLEAN AS $$ DECLARE required_permission TEXT;

has_perm BOOLEAN;

BEGIN required_permission := required_resource || ':' || required_action;

-- Check if user has the exact permission or a matching wildcard permission
-- First check direct permissions (user_permissions), then role-based permissions
SELECT
    EXISTS(
        SELECT
            1
