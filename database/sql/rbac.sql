...
-- Returns all permissions for a user (from roles and direct permissions)
SELECT
    DISTINCT p.*
FROM
    (
...
-- Supports wildcard matching (checks both direct permissions and role-based permissions)
SELECT
    EXISTS(
        SELECT
            1
...
-- Returns only direct permissions assigned to a user (not from roles)
SELECT
    p.*
FROM
    permissions p
...
-- Remove a direct permission from a user (soft delete)
UPDATE
    user_permissions
SET
    deleted_at = NOW()
...
-- name: ClearUserDirectPermissions :exec
-- Remove all direct permissions from a user (soft delete)
UPDATE
    user_permissions
SET
    deleted_at = NOW()
